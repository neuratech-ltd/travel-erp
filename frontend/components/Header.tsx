...
export default function Header({ title, onSearch, onRefresh, isLoading = false }: HeaderProps) {
  ...
  return (
    <header
      id="header-container"
      className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-8 shrink-0 z-10 shadow-sm"
    >
      <div id="header-title-block" className="flex items-center gap-3">
        <h2 id="header-title" className="text-xl font-bold text-slate-800 tracking-tight">
          {title}
        </h2>
        {isLoading && (
          <span
            id="loading-spinner-wrapper"
            className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full animate-pulse"
          >
      ...
      <div id="header-right-actions" className="flex items-center gap-4">
        <div
          id="header-clock"
          className="flex items-center gap-1.5 text-xs text-slate-500 font-medium bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200/50"
        >
        ...
        <div id="header-profile-badge" className="flex items-center gap-2 border-l border-slate-200 pl-4">
          <div
            id="profile-avatar"
            className="h-9 w-9 bg-slate-100 border border-slate-200 rounded-full flex items-center justify-center font-bold text-slate-700 shadow-inner"
          >
