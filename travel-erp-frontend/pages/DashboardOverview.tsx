  ...
  return (
    <div id="dashboard-overview-container" className="flex-1 p-8 overflow-y-auto space-y-8 bg-slate-50">
      ...
      <div id="remittance-sec-container" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          id="rhc-card"
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 lg:col-span-1 flex flex-col justify-between"
        >
          <div>
            <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-3">
              ...
              <span className="text-4xs font-bold text-slate-400 bg-slate-50 px-2.5 py-0.5 rounded-full border border-slate-100 uppercase tracking-widest">
                IATA BSP
              </span>
            </div>
            ...
            <div id="rhc-gauge-wrapper" className="flex flex-col items-center justify-center pt-2">
              ...
              <div className="relative w-48 h-26 flex items-center justify-center overflow-hidden mb-2">
                <svg className="absolute top-0 left-0 w-full h-full filter drop-shadow-sm" viewBox="0 0 100 50">
                  ...
                  <g
                    transform={`rotate(${(remittancePercentage / 100) * 180}, 50, 46)`}
              ...
              <p className="text-4xs text-slate-400 mt-0.5 mb-5 text-center font-semibold">
              ...
              <div className="w-full space-y-2.5 border-t border-slate-100 pt-4 text-2xs text-slate-600">
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-1.5 text-slate-500 font-medium">
                    <ShieldCheck className="h-3.5 w-3.5 text-slate-400" /> RHC Limit Amount
                  </span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-extrabold text-amber-600 font-mono text-xs">
                    ৳{((5000000 * remittancePercentage) / 100).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center">
        ...
        <div
          id="vendors-remittance-card"
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 lg:col-span-2 flex flex-col justify-between"
        >
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-slate-800 text-sm tracking-tight uppercase">Vendors Wise Remittance</h3>
              <span className="text-3xs font-semibold bg-slate-100 text-slate-600 px-2 py-1 rounded-md uppercase">
                Live
              </span>
            </div>
      ...
      <div
        id="medical-tourism-summary-board"
        className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60 space-y-6"
      >
        <div className="flex justify-between items-center border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-3xs text-slate-400 font-medium mt-0.5">
              Real-time status of patients, medical visas, and treatments
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-4xs font-bold text-slate-500">
              ACTIVE CASES: <strong className="text-slate-800 text-2xs">{Math.max(1, 2)}</strong>
            </span>
            <span className="h-2.5 w-px bg-slate-200"></span>
            <span className="text-4xs font-bold text-slate-500">
              MEDICAL SALES:{' '}
              <strong className="text-emerald-600 text-2xs">৳{Math.max(145000, 2 * 10).toLocaleString()}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <TourismOverviewCard
            title={'Hospital Booking Value'}
            ammount={145000}
            description="Direct corporate and personal healthcare treatment invoices"
          />
          <TourismOverviewCard
            title={'Medical Agency Margin'}
            ammount={20000}
            description="Welcare Trip premium commission & consultation margin"
          />
          <TourismOverviewCard
            title={'Treatment Destinations'}
            list={[
              { label: 'Singapore', value: 1 },
              { label: 'Thailand', value: 2 },
              { label: 'India', value: 3 },
            ]}
            description="Patients currently under treatment or consultation"
          />
        </div>
      ...
      <div id="bsp-billing-container" className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/60">
        <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
          ...
          <div className="flex items-center gap-3 text-3xs font-medium text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5">
            <span>
              Sales Period: <strong className="text-slate-800">01-JUL-2026 to 15-JUL-2026</strong>
            </span>
            <span className="text-slate-300">|</span>
            <span>
              Billing Period: <strong className="text-slate-800">16-JUL-2026 to 31-JUL-2026</strong>
            </span>
          </div>
        </div>
