...
export default function AiConsultant() {
  ...
  const quickPrompts = [
    {
      title: 'Singapore 5-Day package',
      prompt:
        'Draft a premium 5-day tour package itinerary for Singapore including hotel Marina Bay Sands, transport transfers, and sightseeing tours.',
    },
    {
      title: 'Refund Email Draft',
      prompt:
        'Compose a professional client refund notification email for cancelled Turkish Airlines flight TK-235 because of air traffic disruption.',
    },
    {
      title: 'IATA BSP explanation',
      prompt:
        'Explain how IATA BSP (Billing and Settlement Plan) upcoming fortnightly billing capacity is calculated and how to manage Remittance Holding Capacity limits.',
    },
    {
      title: 'Check Visa requirements',
      prompt: 'Summarize current tourist visa document requirements for citizens traveling to Japan.',
    },
  ]
  ...
  return (
    <div
      id="ai-consultant-container"
      className="flex-1 flex flex-col md:flex-row h-[calc(100-4rem)] bg-slate-50 overflow-hidden"
    >
      {/* Sidebar Suggestions */}
      <div
        id="ai-quick-sidebar"
        className="w-full md:w-80 bg-white border-b md:border-b-0 md:border-r border-slate-200 p-6 flex flex-col justify-between shrink-0 overflow-y-auto"
      >
        <div className="space-y-4">
          ...
          <p className="text-3xs text-slate-400 font-medium">
            Select a predefined ERP workspace template below to query Gemini immediately:
          </p>
          ...
          <div className="space-y-2.5">
            {quickPrompts.map((p, idx) => (
              ...
              >
                <span className="font-bold text-slate-700 block group-hover:text-indigo-600 transition-colors mb-1">
                  {p.title}
                </span>
        ...
        <div className="mt-8 bg-indigo-50/50 border border-indigo-100 p-4 rounded-xl text-4xs leading-relaxed text-indigo-700 font-medium">
          ...
          To auto-fill invoices with simulated passenger data, use the{' '}
          <strong className="text-indigo-900">"AI Auto-Fill Form"</strong> button directly inside any invoice creator
          form.
        </div>
      </div>
      ...
      <div id="ai-chat-main" className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4">
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex gap-3 max-w-4xl ${isAi ? 'mr-auto' : 'ml-auto flex-row-reverse'}`}>
                {/* Avatar */}
                <div
                  className={`h-8 w-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold ${isAi ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-700'}`}
                >
                ...
                {/* Message Bubble */}
                <div
                  className={`p-4 rounded-2xl border text-2xs leading-relaxed ${
                    isAi
