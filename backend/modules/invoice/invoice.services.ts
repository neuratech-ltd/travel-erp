...
const normalizeContactInput = (value?: InvoiceContactInput | string, email?: string) => {
  if (!value) {
    return undefined
  }
...
const resolveOrCreateClient = async (client?: InvoiceContactInput) => {
  ...
  const createdClient = await prisma.client.create({
    data: {
      name: client.name,
      email: client.email ?? `${slugify(client.name)}-${Date.now()}@travel-erp.local`,
    },
  })
...
const resolveOrCreateEmployee = async (employee?: InvoiceContactInput) => {
  ...
  const createdEmployee = await prisma.employee.create({
    data: {
      name: employee.name,
      email: employee.email ?? `${slugify(employee.name)}-${Date.now()}@travel-erp.local`,
    },
  })
...
const resolveOrCreateAgent = async (agent?: InvoiceContactInput) => {
  ...
  const createdAgent = await prisma.agent.create({
    data: {
      ...
      email: agent.email ?? `${slugify(agent.name as string)}-${Date.now()}@travel-erp.local`,
    },
  })
...
const buildCoreInvoiceData = async (data: BaseInvoiceInputs, invoiceType: InvoiceType) => {
  const clientRef = normalizeContactInput(data.client ?? data.clientName, data.clientEmail)
  const employeeRef = normalizeContactInput(data.employee ?? data.salesBy, data.salesByEmail)
  ...
  const employeeId = data.employeeId ?? (await resolveOrCreateEmployee(employeeRef))
...
export async function createHotelVisaInvoice(data: HotelVisaInputs) {
  const baseData = await buildCoreInvoiceData(data, data.bookingType === 'Hotel' ? InvoiceType.HOTEL : InvoiceType.VISA)
  ...
  const profitValue = toNumber(
    data.profit ?? clientPriceValue - purchasePriceValue + toNumber(data.extraFee) - toNumber(data.discount),
  )
  ...
  return prisma.invoice.create({
    data: {
      ...
      route:
        data.bookingType === 'Hotel'
          ? data.hotelName
          : (data.route ?? (data.visaCountry ? `Visa: ${data.visaCountry}` : undefined)),
...
export async function createNonCommissionInvoice(data: NonCommissionInputs) {
  const vendorId = data.airline ? await resolveOrCreateVendor(data.airline) : undefined
  const baseData = await buildCoreInvoiceData(data, InvoiceType.NON_COMMISSION)
  const profit =
    toNumber(data.clientPrice) - toNumber(data.purchasePrice) + toNumber(data.extraFee) - toNumber(data.discount)
...
export async function createReissueInvoice(data: ReissueInputs) {
  ...
  const purchasePrice = toNumber(data.penalties) + toNumber(data.fareDifference) + toNumber(data.taxDifference)
  const clientPrice = purchasePrice + toNumber(data.extraFee) - toNumber(data.discount)
...
export async function createTourPackageInvoice(data: TourPackageInputs) {
  ...
  const accommodation = data.accommodation
    ? {
        ...
        nights: data.accommodation.nights === undefined ? undefined : toNumber(data.accommodation.nights),
  ...
  const billing = data.billing
    ? {
        ...
        billingQty:
          data.billing.billingQty === undefined ? undefined : Math.trunc(toNumber(data.billing.billingQty, 1)),
  ...
  const profit =
    billing?.totalProfit ??
    (clientPrice !== undefined && purchasePrice !== undefined ? clientPrice - purchasePrice : undefined)
  ...
  return prisma.invoice.create({
    data: {
      ...
      purchasePrice: purchasePrice === undefined ? undefined : toNumber(purchasePrice),
      clientPrice: clientPrice === undefined ? undefined : toNumber(clientPrice),
