  ...
  try {
    ...
    const rows: GeneratedSalesReportRow[] = invoices.map((invoice) => {
      ...
      const totalSales = toMoney(
        ticketReissue + admaVoidCharge + visaAppFee + hotelBooking + ticket,
      );
        ...
        invoice.payments
          .filter(
            (payment) =>
              ...
              normalizeBankChannel(payment.bankChannel).includes("BRAC"),
          )
          .reduce((sum, payment) => sum + payment.amount, 0),
      );
      ...
      const bankPubali = toMoney(
        invoice.payments
          .filter(
            (payment) =>
              payment.method === PaymentMethod.BANK &&
              normalizeBankChannel(payment.bankChannel).includes("PUBALI"),
          )
          .reduce((sum, payment) => sum + payment.amount, 0),
      );
      ...
      const bankDbbl = toMoney(
        invoice.payments
          .filter(
            (payment) =>
              payment.method === PaymentMethod.BANK &&
              normalizeBankChannel(payment.bankChannel).includes("DBBL"),
          )
          .reduce((sum, payment) => sum + payment.amount, 0),
      );
